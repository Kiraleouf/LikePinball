...
### Templates réservés à un index

Dans Sector Lab, **Nouveau** crée un template ; le champ **Index fixe** peut rester vide (pool générique) ou contenir un entier, par exemple `10`. Le départ est l’index `0` ; l’index `10` est donc le onzième secteur physique. **Sauvegarder pour les runs** active le template et **Template actif** permet de le rouvrir. Effacer l’index d’un template non structurel le remet dans le pool générique après sauvegarde. **Retirer des runs** désactive un template personnalisé. Le secteur initial conserve obligatoirement l’index `0`.
